...
namespace {
void Random(std::vector<kalinin_d_jarvis_convex_hull_tbb::Point> &points,
            std::vector<kalinin_d_jarvis_convex_hull_tbb::Point> &hull,
            std::vector<kalinin_d_jarvis_convex_hull_tbb::Point> &res_hull) {
...
TEST(kalinin_d_jarvis_convex_hull_tbb, Random_Points_RNG) {
  std::random_device dev;
  std::mt19937 rng(dev());
  std::uniform_int_distribution<int> dist(-8, 8);

  // Create data
  auto gen = [&dist, &rng]() { return kalinin_d_jarvis_convex_hull_tbb::Point{.x = dist(rng), .y = dist(rng)}; };

  std::vector<kalinin_d_jarvis_convex_hull_tbb::Point> points(30);
